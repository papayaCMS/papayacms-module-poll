...
class base_poll extends base_db {
  ...
  */
  function categIsEmpty($id) {
    $sql = "SELECT COUNT(*) FROM %s WHERE categ_id = '%d'";
  ...
  */
  function userCanVote() {
    ...
    if ($this->papaya()->surfer->isValid) {
      $userId = $this->papaya()->surfer->surferId;
      $sql = "SELECT COUNT(*)
                FROM %s
    ...
    if (isset($this->answers) && is_array($this->answers)) {
      foreach ($this->answers as $answerId => $answer) {
        ...
        if (isset($this->results) && is_array($this->results)) {
          foreach ($this->results as $result) {
            if ($result['answer_id'] == $answerId) {
              $this->databaseDeleteRecord($this->tableResults, 'entry_id', $answerId);
            }
          }
        }
        if ($answer['poll_id'] == $i) {
